...
def plot_train_dynamics(losses, accs):
    plt.plot(list(range(1, len(accs) + 1)), accs, color='blue', label='accuracy')
    plt.plot(list(range(1, len(accs) + 1)), losses, color='red', label='loss')
    plt.legend()
    plt.xlabel('Epoch')
    plt.show()
    plt.savefig('C:\\Users\\micha\\OneDrive\\Skrivebord\\dtu_mlops\\Corrupted_MNIST\\reports\\figures\\train_dynamics.png')
...
def train(lr):
    ...
    plot_train_dynamics(accuracies, mean_losses)
    return accuracies, mean_losses
